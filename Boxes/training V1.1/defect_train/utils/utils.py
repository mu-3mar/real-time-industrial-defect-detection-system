...
def check_and_download_model(model_path: Path):
    ...
    # Custom checkpoints (e.g. defect_detector.pt) should be provided manually.
    if not model_name.startswith(("yolov8", "yolo11", "yolo12", "yolo26")):
        raise FileNotFoundError(
            f"Missing custom pretrained checkpoint: {model_path}. "
            "Put your .pt file there or set DEFECT_TRAIN_BASE_MODEL."
        )

    logger.info("Attempting download from Ultralytics assets...")

    # Ensure parent directory exists
    model_path.parent.mkdir(parents=True, exist_ok=True)

    # Construct URL for standard YOLO model weights.
