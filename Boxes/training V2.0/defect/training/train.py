...
from configs.config import (
    ROOT, PROJECT_DIR, DATA_YAML, DATA_DIR, BASE_MODEL, 
    EPOCHS, BATCH_SIZE, IMG_SIZE, DEVICE, PROJECT_NAME
)
from utils.utils import ensure_dirs, prune_weights, check_and_download_model, write_data_yaml, validate_dataset
...
def train(fresh: bool = False):
    ...
    checkpoint = None if fresh else _latest_checkpoint()
    ...
    if resume_training:
        ...
        if fresh:
            logger.info("Fresh training requested (ignoring any saved checkpoints)")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--fresh",
        action="store_true",
        help="Start from pretrained weights, ignore any saved checkpoint",
    )
