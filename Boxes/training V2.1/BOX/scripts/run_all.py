...
from export.export_tensorrt import export_tensorrt
...
def main():
    logger.info("Starting full box_detector pipeline...")
    ...
    # 3. Quantize ONNX
    logger.info("Step 3: Quantizing ONNX model to INT8")
    quantize(run_dir, onnx_path)
    
    # 4. Export to TensorRT
    logger.info("Step 4: Exporting model to TensorRT format")
    try:
        export_tensorrt(onnx_path=onnx_path, run_dir=run_dir)
    except Exception as e:
        logger.error(f"TensorRT export failed (requires CUDA/TensorRT installed): {e}")
        logger.warning("Continuing without TensorRT export...")
    
    # 5. Collect Metrics
    logger.info("Step 5: Collecting training metrics")
