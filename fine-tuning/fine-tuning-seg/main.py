...
def train():
    ...
    cfg = {"data": "data/data.yaml",
        "imgsz": 640, "batch": 2, "device": "0",  # ⬅️ غير 8 لـ 2
        "project": str(OUT), "name": "train",
        "exist_ok": True, "plots": False, "val": False}

    start = read_epoch() + 1
    if start > 50:                      # 50 is our total
        print("Training already finished."); return

    best_map = 0.0
    for epoch in range(start, 51):      # 1 … 50
        print(f"\n----- epoch {epoch}/50 -----")

        # 🔥 التغيير الوحيد: استبدل yolov8s.pt بـ yolov8s-seg.pt
        model = YOLO(str(OUT / "last.pt")) if epoch > 1 else YOLO("yolov8n-seg.pt")
    ...
    # 🔥 التغيير الثاني: استبدل metrics.box.map بـ metrics.seg.map
